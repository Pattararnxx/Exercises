...
async function loadPokemon() {
    ...
    for(let i = 0; i < data.results.length; i++) {
        ...
        const types = pokemonDetails.types.map(t => `
          <div class="mb-2">
            <span class="inline-block bg-white bg-opacity-20 text-white text-2xl font-normal px-5 py-2 rounded-full">
              ${t.type.name}
            </span>
          </div>
        `).join('');
        ...
        card.className = `h-[400px] ${bgColor} rounded-4xl shadow-lg pl-15 flex justify-between  shadow:${bgColor} relative overflow-hidden`;
        ...
        card.innerHTML = `
          <div class="z-10 pt-15 w-[40%]">
            <h2 class="text-white text-5xl font-extrabold mb-10 capitalize">${pokemonDetails.name}</h2>
            <div>${types}</div>
          </div>
          <div class="z-10 w-[50%] flex justify-center items-center">
            <img src="${pokemonDetails.sprites.other['official-artwork'].front_default}" alt="" class="w-full h-auto max-h-[300px] object-contain" />
          </div>
          <p class="absolute top-3 right-5 text-white text-6xl font-normal opacity-20">#${pokemonDetails.id.toString().padStart(3, '0')}</p>
          <img class="absolute -bottom-15 -right-15 opacity-30 w-[300px] h-auto " src="../images/pokeball.png" alt="">
        `;
        ...
        card.addEventListener('click', () => {
           ...
           content.innerHTML = `
            <div class="relative z-60 ${bgColor} w-260 h-auto overflow-hidden">
                <div class=" flex flex-col juastify-center items-center text-center ">
                    <div class="z-10 pt-15 w-[40%]">
                        <h2 class="text-white text-5xl font-extrabold mb-10 capitalize">${pokemonDetails.name}</h2>
                        <div class="flex justify-center gap-2 mb-40">${types}</div>
                    </div>
                    <div class="z-10 w-[50%] flex justify-center items-center">
                    <img src="${pokemonDetails.sprites.other['official-artwork'].front_default}" 
                        alt="" 
                        class="absolute z-20 top-55 w-full h-auto max-h-[200px] object-contain" />
                    </div>
                </div>
                <p class="absolute z-70 top-10 right-10 text-white text-6xl font-medium opacity-20">#${pokemonDetails.id.toString().padStart(3, '0')}</p>
                <img class="absolute top-50 -right-10 opacity-30 w-[250px] h-auto " src="../images/pokeball.png" alt="">
                <div class="flex flex-col justify-items-start items-start p-10 rounded-[50px] bg-white shadow-[0_10px_30px_rgba(0,0,0,0.6)]">
                    <div class="text-2xl font-bold text-center text-gray-500 border-b border-gray-200 w-full">
                        <ul class="flex flex-wrap -mb-px">
                            <li class="me-2" >
                                <a href="#" class="tab-button inline-block p-4 border-b-2 border-transparent rounded-t-lg hover:text-black hover:border-gray-300 ">About</a>
                            </li>
                            <li class="me-2">
                                <a href="#" class="tab-button inline-block p-4 border-b-2 border-transparent rounded-t-lg hover:text-black hover:border-gray-300" aria-current="page">Base Stats</a>
                            </li>
                            <li class="me-2">
                                <a href="#" class="tab-button inline-block p-4 border-b-2 border-transparent rounded-t-lg hover:text-black hover:border-gray-300 ">Evolution</a>
                            </li>
                        </ul>
                    </div>
                    <div class="tab-content w-220 flex flex-col gap-6 m-6 text-2xl" id="tab-content">
               ...
               button.addEventListener('click', (e) => {
                   ...
                   if(selectedTab === 'about') {
                       ...
                       <div class="flex gap-30">
                            <h2 class="text-gray-700 ">Species</h2>
                            <div class="font-bold">${pokemonDetails.types.map(t=> `
                                <span>${t.type.name}</span>
                            `).join(',')}
                            </div>
                       </div>
                       <div class="flex gap-32">
                            <h2 class="text-gray-700">Height</h2>
                            <div class="font-bold">${pokemonDetails.height}<span>0cm</span></div>
                       </div>
                       <div class="flex gap-32">
                            <h2 class="text-gray-700">Weight</h2>
                            <div class="font-bold">${pokemonDetails.weight/10} <span>kg</span></div>
                       </div>
                       <div class="flex gap-30">
                            <h2 class="text-gray-700">Abilities</h2>
                            <div class="font-bold">${pokemonDetails.abilities.map(a=> `
                           ...
                           return `
                            <div class="flex1 flex justify-between text-center items-center mb-4 pb-2">
                                ...
                                <div class="flex justify-center items-center gap-8">
                                    <div class="font-bold">${s.base_stat}</div>
                                    <div class="w-150 bg-gray-200 rounded-full h-1.5">
